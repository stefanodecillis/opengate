...
export async function installSkill(client: Client, scope: Scope): Promise<string | null> {
  ...
  if (client === 'claude-code') {
    ...
    const filePath = join(dir, 'opengate.md')
    await writeFile(filePath, SKILL_CONTENT)
    return filePath
  }

  if (client === 'opencode') {
    const dir = join(process.cwd(), '.opencode', 'skills')
    await mkdir(dir, { recursive: true })
    const filePath = join(dir, 'opengate.md')
    await writeFile(filePath, SKILL_CONTENT)
    return filePath
  ...
  if (client === 'openclaw') {
    const dir = join(homedir(), '.openclaw', 'skills', 'opengate')
    await mkdir(dir, { recursive: true })
    const filePath = join(dir, 'SKILL.md')
    const content = await resolveOpenClawSkill()
...
/**
 * Resolve the OpenClaw skill content.
 * Tries the installed @opengate/openclaw package first, then falls back
 * to the plugin package within this monorepo, then to inline content.
 */
async function resolveOpenClawSkill(): Promise<string> {
  const candidates = [
    // Installed via npm
    resolvePackagePath('@opengate/openclaw', 'skills/opengate/SKILL.md'),
    // Within this monorepo (dev / CI)
    join(new URL('../../openclaw-plugin/skills/opengate/SKILL.md', import.meta.url).pathname),
  ]
  ...
  // Fallback: inline with OpenClaw-specific frontmatter
  return SKILL_CONTENT.replace(
    /^---\n/,
    '---\nuser-invocable: false\nmetadata:\n  always: true\n',
  )
}

function resolvePackagePath(pkg: string, relativePath: string): string | null {
  try {
    const require = createRequire(import.meta.url)
    const pkgMain = require.resolve(pkg)
    // Walk up to package root
    const parts = pkgMain.split('node_modules/' + pkg)
    if (parts.length < 2) return null
    return join(parts[0], 'node_modules', pkg, relativePath)
  } catch {
    return null
  }
}
